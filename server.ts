...
- Item References: When recommending items, include their full name and highlight why it pairs well or fits their desire.`;
    ...
    // Extract item IDs mentioned or relevant
    const allIds = [
      'kanz-sourdough-boule', 'pistachio-kunafa-croissant', 'classic-baklava-trio',
      'zaatar-olive-focaccia', 'rosewater-pistachio-cake', 'halloumi-pesto-croissant',
      'date-walnut-maamoul', 'kanz-cardamom-latte', 'saffron-milk-cake',
      'karak-tea-pot', 'sesame-simit-ring', 'basbousa-cream-tart'
    ];

    const lower = sanitizedText.toLowerCase();
    const matchedIds = allIds.filter(id => lower.includes(id) || lower.includes(id.replace(/-/g, ' ')));
    const recommendedItemIds = matchedIds.length > 0 ? matchedIds : ['pistachio-kunafa-croissant', 'kanz-sourdough-boule'];
